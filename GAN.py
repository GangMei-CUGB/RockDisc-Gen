...
def train_gan(data_file='Oernlia_set1.csv',
              norm_file='G_Standard_first_dataset.csv',
              output_file='G_prediction.csv',
              g_model_path='generator.pth',
              d_model_path='discriminator.pth',
              g_losses_file='g_loss.csv',
              d_losses_file='d_loss.csv',
              z_dim=100,
              batch_size=8,
              num_epochs=200,
              plot=False,
              save_directory='./work'):
    dataset, scaler = normalize_data(data_file, os.path.join(save_directory, norm_file))
    ...
    save_to_csv(save_directory, output_file, prediction)
    plot_circular_structural_planes(os.path.join(save_directory, output_file), save_directory)
    save_losses(save_directory, g_losses_file, g_losses)
    save_losses(save_directory, d_losses_file, d_losses)

    save_model(save_directory, G, g_model_path)
    save_model(save_directory, D, d_model_path)
    ...
    plt.ylabel('Loss')
    plt.grid()
    plt.savefig(os.path.join(save_directory, 'GAN_training_loss.png'))
    if plot:
        plt.show()
