...
from base_method.boxplot.DDPM_boxplot_lib import DDPM_plot_box_comparison1, DDPM_plot_box_comparison2, DDPM_plot_box_comparison3
from base_method.boxplot.DDPM_histogram import plot_dip_direction_histogram, plot_dip_angle_histogram, plot_trail_length_histogram
...
def run_diffusion_training(input_csv, output_dir, data_num=900, num_epochs=200, plot=True):
    ...
    dip_direction = np.array(df[['dip direction']])
    dip_angle = np.array(df[['dip angle']])
    trail_length = np.array(df[['trail length']])
    print("原始数据统计信息:")
    ...
    prediction_df.to_csv(os.path.join(output_dir, 'DDPM_prediction.csv'), index=False)
    
    # 调用绘图函数
    csv_path = os.path.join(output_dir, 'DDPM_prediction.csv')
    plot_circular_structural_planes(csv_path, output_dir)
    DDPM_plot_box_comparison1(input_csv, csv_path, output_dir)
    DDPM_plot_box_comparison2(input_csv, csv_path, output_dir)
    DDPM_plot_box_comparison3(input_csv, csv_path, output_dir)
    plot_dip_direction_histogram(input_csv, csv_path, output_dir)
    plot_dip_angle_histogram(input_csv, csv_path, output_dir)
    plot_trail_length_histogram(input_csv, csv_path, output_dir)
